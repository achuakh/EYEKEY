...
class RVMWorker(QThread):
    ...
    def run(self):
        try:
            ...
            with open("rvm_a6000.engine", "rb") as f:
                engine = runtime.deserialize_cuda_engine(f.read())
            context = engine.create_execution_context()
            
            # 2. PRE-ALLOCATE GPU BUFFERS (Static 1080p for A6000)
            src_gpu = torch.empty((1, 3, 1080, 1920), dtype=torch.float16, device='cuda')
            fgr = torch.empty((1, 3, 1080, 1920), dtype=torch.float16, device='cuda')
            pha = torch.empty((1, 1, 1080, 1920), dtype=torch.float16, device='cuda')
            
            # Recurrent States - Input (Read Memory)
            states = {
                'r1': torch.zeros((1, 16, 540, 960), dtype=torch.float16, device='cuda'),
                'r2': torch.zeros((1, 20, 270, 480), dtype=torch.float16, device='cuda'),
                'r3': torch.zeros((1, 40, 135, 240), dtype=torch.float16, device='cuda'),
                'r4': torch.zeros((1, 64, 68, 120), dtype=torch.float16, device='cuda')
            }

            # Recurrent States - Output (Write Memory)
            out_states = {
                'ro1': torch.zeros_like(states['r1']),
                'ro2': torch.zeros_like(states['r2']),
                'ro3': torch.zeros_like(states['r3']),
                'ro4': torch.zeros_like(states['r4'])
            }
            ...
            self.status_signal.emit("LIVE: RVM-TRT")
            ...
            while self.running:
                ...
                # 1. GPU UPLOAD (Zero-Copy)
                raw_pixels = np.frombuffer(v.data, dtype=np.uint8).reshape(h, w, 4)
                frame_tensor = torch.from_numpy(raw_pixels).to('cuda', non_blocking=True)
                src_gpu.copy_(frame_tensor[:, :, :3].permute(2, 0, 1).unsqueeze(0).half() / 255.0)

                # 2. SET ALL TENSOR ADDRESSES (Mandatory for TRT 10)
                context.set_tensor_address("src", src_gpu.data_ptr())
                context.set_tensor_address("fgr", fgr.data_ptr())
                context.set_tensor_address("pha", pha.data_ptr())
                
                for i in range(1, 5):
                    context.set_tensor_address(f"r{i}", states[f'r{i}'].data_ptr())
                    context.set_tensor_address(f"ro{i}", out_states[f'ro{i}'].data_ptr())

                # 3. EXECUTION
                context.execute_async_v3(torch.cuda.current_stream().cuda_stream)
                torch.cuda.current_stream().synchronize()

                # 4. SWAP STATES (Ping-Pong for next frame)
                for i in range(1, 5):
                    states[f'r{i}'].copy_(out_states[f'ro{i}'])
               
                # 5. ALPHA REFINEMENT (Using pha generated by TRT)
